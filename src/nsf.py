...
class Nsf(scraper.Scraper):

    def __init__(self, start, end, inst):
        super().__init__(start,end)
        self.start =  datetime.datetime.strptime(start, '%Y%m%d').strftime('%m/%d/%Y')
        self.end = datetime.datetime.strptime(end, '%Y%m%d').strftime('%m/%d/%Y')
        self.data = []
        self.inst = inst
    ...
    def search_by_date_range(self):
        ...
        while True:
            ...
            query_parameters = ''.join([ '?', 'startDateStart=', self.start, 
                                        '&', 'startDateEnd=', self.end,
                                        '&', 'awardeeName="', self.inst, '"',
            ...
            if len(response.json()['response']['award']) == 25:
                ...
                self.award_id_list = award_id_list
                return

        
    def retrieve_award_info(self):
        """
        ...
        for item in self.award_id_list:
            http = requests.Session()
            adapter = HTTPAdapter(max_retries=5)
            adapter.max_retries.respec_retry_after_header = False
            http.mount('http://', adapter)
        ...
        for item in award_dict.keys():
            myObj = {
                "id" : award_dict[item]['id'],
                "agency": award_dict[item]['agency'],
                "awardeeName": award_dict[item]['awardeeName'],
                "piFirstName": award_dict[item]['piFirstName'],
                "piLastName": award_dict[item]['piLastName'],
                "coPDPI": json.dumps(award_dict[item]['coPDPI']),
                "pdPIName": award_dict[item]['pdPIName'],
                "startDate": award_dict[item]['startDate'],
                "expDate": award_dict[item]['expDate'],
                "estimatedTotalAmt": award_dict[item]['estimatedTotalAmt'],
                "title": award_dict[item]['title']
            }
